...
// My code contribution is this function
int* matchingStrings(int strings_count, char** strings, int queries_count, char** queries, int* result_count) {
    ...
    // The size of the results array will be equal to the number of queries
    int* results = (int*)calloc(queries_count, sizeof(int)); // Use calloc to initialize all elements to 0
    if (results == NULL) {
    ...
    // Set the size of the result array for the caller
    *result_count = queries_count;

    // Iterate through each query string
    for (int i = 0; i < queries_count; i++) {
        int count = 0; // Initialize count for the current query
        // Iterate through each input string
        for (int j = 0; j < strings_count; j++) {
            // Compare the current query string with the current input string
            // strcmp returns 0 if the strings are identical
            if (strcmp(queries[i], strings[j]) == 0) {
                count++; // Increment count if a match is found
            }
        }
        // Store the count for the current query in the results array
        results[i] = count;
    }

    // Return the dynamically allocated results array
    return results;
}
