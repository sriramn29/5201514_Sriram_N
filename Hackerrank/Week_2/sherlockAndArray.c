...
char* balancedSums(int arr_count, int* arr) {
    ...
    // Iterate through the array once to find a potential balance point.
    for (int i = 0; i < arr_count; i++) {
        // The right sum is the total sum minus the left sum and the current element.
        long long right_sum = total_sum - left_sum - arr[i];
        
        // If the left sum equals the right sum, a balance point is found.
        if (left_sum == right_sum) {
            return "YES";
        }
    ...
    // If no balance point is found after iterating through the entire array.
    return "NO";
}
